...
class Uploader extends Component {
  ...
  render() {
    ...
    return (
      <div className="upload">
        <span className="upload-button" onClick={this.showUpload}>
          <i className="material-icons">cloud_upload</i> {t("Upload")}
        </span>
        <UploaderModal
  ...
  handleFiles = files => {
    ...
    this.setState(
      { images: new Map([...this.state.images, ...tempMap]) },
      // Upload
      () => {
    ...
    query("images", {
      ...
      .then(response => {
        ...
        if (this.state.images.size === 1) {
          // Go to image
          this.setState({ redirect: `/i/${response.identifier}` });
          this.resetState();
        } else {
          // Add to temp album
          this.setState({
      ...
      .catch(e => {
        this.setState({
          images: new Map([
            ...this.state.images,
            [index, Object.assign({}, image, { status: this.status_fail })]
          ])
        });
      });
  };
  ...
  createAlbum = () => {
    ...
    query("albums", { method: "POST", body }).then(response => {
      ...
      this.setState({ redirect: `/a/${response.identifier}` });
      this.resetState();
    });
  };
